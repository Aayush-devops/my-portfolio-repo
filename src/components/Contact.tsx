...
const Contact: React.FC = () => {
  ...
  // Removed duplicate handleChange function to fix redeclaration error
  ...
  const contactInfo = [
    {
      icon: Mail,
      label: "Email",
      value: "sharmaayush54321@gmail.com",
      href: "mailto:sharmaayush54321@gmail.com",
      gradient: "from-cyan-400 to-blue-500",
    },
    {
      icon: MapPin,
      label: "Location",
      value: "prayagraj, India", // Your actual city/state
      href: "https://maps.google.com/?q=prayagraj,India", // Clickable map link
  ...
  return (
    <section
      id="contact"
      className="py-8 sm:py-12 lg:py-20 bg-gradient-to-b from-slate-900 to-slate-950"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          ...
          <p className="text-sm sm:text-base lg:text-lg text-slate-400 max-w-2xl mx-auto leading-relaxed">
            Ready to discuss your next data engineering project or explore
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      <label
                        htmlFor="name"
                        className="block text-xs sm:text-sm font-semibold text-slate-300 mb-1.5 sm:mb-2"
                      >
                        Your Name
                      </label>
                      <input
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        required
                        className={`w-full px-3 py-2.5 sm:px-4 sm:py-3 bg-slate-800/50 border ${
                          errors.email
                            ? "border-red-500"
                            : "border-slate-700/50"
                        } rounded-lg sm:rounded-xl text-white placeholder-slate-400 
        focus:outline-none focus:border-cyan-400/50 focus:bg-slate-800/70 
        transition-all duration-300 text-sm sm:text-base`}
            ...
            <div className="group relative bg-slate-900/60 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-5 border border-slate-800/60 hover:border-slate-700/60 transition-all duration-300">
              ...
              <div className="relative z-10">
                <h3 className="text-sm sm:text-base lg:text-lg font-bold text-white mb-2 sm:mb-3">
                  Connect With Me
                </h3>
                ...
                <div className="space-y-2 sm:space-y-3">
                  {contactInfo.map((info, index) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-gradient-to-br ${info.gradient} bg-opacity-10 rounded-md sm:rounded-lg group-hover/contact:scale-110 transition-transform duration-300`}
                        >
                          <Icon className="w-6 h-6 text-white" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-[10px] sm:text-xs text-slate-400 font-medium mb-0.5">
                            {info.label}
                          </p>
                          <p className="text-slate-300 font-semibold truncate text-xs sm:text-sm">
                            {info.value}
                          </p>
                        </div>
                      </a>
                ...
                <div className="space-y-2 sm:space-y-3">
                  {personalDetails.map((detail, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="flex items-start gap-2 p-2 sm:p-3 bg-slate-800/20 rounded-lg border border-slate-700/30"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-slate-400 text-[10px] sm:text-xs font-medium mb-0.5">
                            {detail.label}
                          </p>
                          <p className="text-slate-300 font-semibold text-[10px] sm:text-xs leading-relaxed">
                            {detail.value}
                          </p>
                        </div>
                      </div>
            ...
            <div className="group relative bg-slate-900/60 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-5 border border-slate-800/60 hover:border-slate-700/60 transition-all duration-300">
              ...
              <div className="relative z-10">
                ...
                <p className="text-slate-300 leading-relaxed text-[10px] sm:text-xs lg:text-sm">
                  ...
                  for new opportunities and consulting projects. Open to
                  discussing{" "}
                  <span className="text-cyan-400 font-semibold">
                    full-time positions
                  </span>
                  , contract work, and collaborative ventures in data
